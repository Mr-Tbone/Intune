...
#>
    ...
    $diag['PartitionStyle'] = if ($osDisk) { $osDisk.PartitionStyle } else { "Unknown" }
    ...
    $diag['IsVirtualMachine'] = if ($cs -and $cs.Model -match "Virtual|VMware|VirtualBox|Hyper-V|QEMU|Parallels") { "Yes" } elseif ($cs) { "No" } else { "Unknown" }
    # BIOS info
    $bios = Get-CimInstance Win32_BIOS -Property SMBIOSBIOSVersion,ReleaseDate -ErrorAction SilentlyContinue
    $diag['BIOSVersion'] = if ($bios) { $bios.SMBIOSBIOSVersion } else { "Unknown" }
    $diag['BIOSDate'] = if ($bios -and $bios.ReleaseDate) { $bios.ReleaseDate.ToString("yyyy-MM-dd") } else { "Unknown" }
