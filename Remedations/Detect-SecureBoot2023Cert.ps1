...
<#
.SYNOPSIS
    Script for Intune remediation to detect if Secure Boot has certificate Windows UEFI CA 2023 in DB

.DESCRIPTION
    The script detects if Secure Boot has certificate Windows UEFI CA 2023 in DB on the device and also collects diagnostic info on the device.
    Returns Compliant (exit 0) if exist, Non-Compliant (exit 1) if not exist or not supported.
...
$ScriptName = "Detect Secure Boot Windows UEFI CA 2023"                      # Name of the script (Used for log file name)
...
try {
    ...
    if ($AllPreRec) {
        ...
        if ($diagnostics -and $diagnostics.SecureBootDBHas2023 -eq $true) {
            Write-Host "Secure Boot Windows UEFI CA 2023 exist in DB"
            $AllCompliant = $true
        }
        else {
            Write-Warning "Secure Boot Windows UEFI CA 2023 does not exist in DB"
            $AllCompliant = $false
        }
    } else {
